...
本地曲库由 Dexie 管理。文件标签用于首次入库；根目录和相对父文件夹只作为专辑聚合启发式，不属于艺术家或专辑的稳定身份。在线匹配会把候选结果中的标题、艺术家与专辑作为一组元数据应用；实体展示名称与成员关系彼此独立。你可以从 GridView 的实体信息面板修改展示名称，并显式合并或拆分艺术家、专辑实体。
